...
def download_checkpoint(checkpoint_url, output_dir):
  try:
    print(os.path.join(checkpoint_url, MANIFEST_FNAME))
    response = urllib2.urlopen(
        os.path.join(checkpoint_url, MANIFEST_FNAME).replace('\\', '/'))
  ...
  for p in manifest[0]['paths']:
    ...
    try:
      response = urllib2.urlopen(os.path.join(checkpoint_url, p).replace('\\', '/'))
    except urllib2.HTTPError as e:
  ...
  try:
    response = urllib2.urlopen(os.path.join(checkpoint_url, CONFIG_FNAME).replace('\\', '/'))
  except urllib2.HTTPError as e:
